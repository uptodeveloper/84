...
84는 React SPA 기반으로 구현한 중고 거래 플랫폼입니다.

MVP 구현에서 끝내지 않고, React 버전을 기준점으로 정리한 뒤
Next.js App Router 버전을 별도로 구현하며,
데이터의 성격에 따라 서버와 클라이언트의 책임을 나누고
Next.js App Router의 서버 fetch, 이미지 최적화, 라우팅, 캐싱 기능 등을 활용해
코드와 구조를 단순화하면서 사용자 경험 개선을 목표로 하고 있는 프로젝트입니다.
...
- [ ] `v3.0.0-next-app-router`: Next.js App Router 버전 구현

## v2.0.0 React Cleanup

v2는 Next.js App Router 버전을 구현하기 전,
React 버전을 비교 가능한 기준점으로 남기기 위해 필요한 범위만 정리한 단계입니다.
...
- 전체 컴포넌트 구조 개편보다, v3 구현 전에 이해와 유지보수에 필요한 범위만 정리했습니다.
...
- 서버/클라이언트 데이터 패칭 구조 분리
- Next.js 서버 fetch, 이미지 최적화, 캐싱 기능 적용
- 데이터 변경 이후 서버 캐시와 클라이언트 캐시 동기화 방식 검토
